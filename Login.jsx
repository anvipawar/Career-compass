...
export default function Login() {
    ...
    // Debug: Log the client ID (first few chars) to verify it's loaded
    React.useEffect(() => {
        if (!googleClientId) {
            console.error('Google Client ID is missing from .env file!');
        } else {
    ...
    return (
        <div className="min-h-[calc(100vh-64px)] flex items-center justify-center px-6 py-12">
            <div className="glass-card w-full max-w-md p-8 sm:p-10">
                ...
                {/* Google Sign In */}
                {!googleClientId ? (
                    <div className="mb-6 p-6 bg-red-500/10 border-2 border-red-500/50 rounded-xl text-red-500 text-sm font-bold text-center">
                        ⚠️ SYSTEM ERROR: Google Client ID is missing!<br />
                        <span className="text-xs font-normal opacity-80 mt-2 block">
                            Please add VITE_GOOGLE_CLIENT_ID to your Vercel Environment Variables and REDEPLOY the app.
                        </span>
                    </div>
                ) : (
                    <button onClick={handleGoogle} id="google-signin-btn"
                        className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all font-medium text-sm mb-6">
                        <GoogleIcon />
